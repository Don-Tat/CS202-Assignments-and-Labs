public final class ArrayBag<T> implements BagInterface<T> {
   ...
   /*
    * 1. Define an equals method for the class ArrayBag that returns true when the
    * contents of two bags are the same.
    * Note that two equal bags contain the same number of entries, and each entry
    * occurs in each bag the same number of times.
    * The order of the entries in each array is irrelevant.
    */
      ...
      if (other instanceof ArrayBag) {
         ...
         if (this.getCurrentSize() == otherBag.getCurrentSize()) {
            ...
            for (int i = 0; i < this.getCurrentSize(); i++) {
               if (thisBagArray[i] != otherBagArray[i]) {
                  result = false;
                  break;
               } else
                  result = true;
